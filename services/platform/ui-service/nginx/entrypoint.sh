...
if [ -f /var/run/starterkit-ui-overlay/index.html ]; then
  ui_root_target="/var/run/starterkit-ui-overlay"
fi
ln -sfn "${ui_root_target}" /var/run/starterkit-ui-config/web-root
...
envsubst '${UI_PUBLIC_API_URL} ${UI_PUBLIC_ORCHESTRATION_URL} ${UI_PUBLIC_POLICY_URL} ${UI_PUBLIC_OPA_URL} ${UI_PUBLIC_TEMPORAL_UI_URL} ${UI_PUBLIC_JAEGER_URL}' \
  < /etc/starterkit-ui/runtime-config.json.template \
  > /var/run/starterkit-ui-config/config.json
